...
//    if(game_board.at(position.first).at(position.second).is_piece()){ // fails if no piece at position
        if(game_board.at(position.first).at(position.second).piece->type == 'r')
        {
          ...
          else
          {
            ...
            {
              // if (rings_vector.at(i) == position) {
              if ((opp_rings_vector.at(i).first == position.first) & (opp_rings_vector.at(i).second == position.second)) {
...
//    if(already_valid || is_move_valid(p1, p2)) {
      ...
      game_board.at(p1.first).at(p1.second).piece = new Piece('m', game_board.at(p1.first).at(p1.second).piece->color, p1.first, p1.second);
...
vector< pair< pair<int,int>, pair<int,int> > > Board::successors(pair<int,int> initial_pos) {
    ...
    i = 1;
    while(true) {
        if(game_board.find(initial_pos.first + i) == game_board.end() || game_board.find(initial_pos.first + i)->second.find(initial_pos.second) == game_board.find(initial_pos.first + i)->second.end())
            break;
    ...
    i = 1;
    while(true) {
        if(game_board.find(initial_pos.first + i) == game_board.end() || game_board.find(initial_pos.first + i)->second.find(initial_pos.second + i) == game_board.find(initial_pos.first + i)->second.end())
            break;
...
void Board::execute_move(string move, int playerID)
{
	...
	if (splitString.size() == 3) { // Place a ring
        ...
        pair<int, int> start_hex = make_pair( stoi( splitString.at(1)), stoi(splitString.at(2)) ); //start position in hex coordinates
    ...
    for(int x = -(n-1); x<= (n-1); x++) {
        ...
        for(int y = bounds.first; y <= bounds.second; ) {
            if(game_board.at(x).at(y).is_marker() && game_board.at(x).at(y).piece->color == color) {
                int i = 1;
                while(true){
                    if(y+i > bounds.second || !game_board.at(x).at(y+i).is_marker() || game_board.at(x).at(y+i).piece->color != color)
                        break;
                    i++;
                }
                if(i>length-1)
    ...
    for(int y = -(n-1); y<= (n-1); y++) {
        ...
        for(int x = bounds.first; x <= bounds.second; ) {
            if(game_board.at(x).at(y).is_marker() && game_board.at(x).at(y).piece->color == color) {
                int i = 1;
                while(true){
                    if(x+i > bounds.second || !game_board.at(x+i).at(y).is_marker() || game_board.at(x+i).at(y).piece->color != color)
                        break;
                    i++;
                }
                if(i>length-1)
    ...
    for(int k = -(n-1); k <= (n-1); k++){
        ...
        for(int x = bounds.first; x <= bounds.second; ){
            if(game_board.at(x).at(x-k).is_marker() && game_board.at(x).at(x-k).piece->color == color) {
                int i = 1;
                while(true){
                    if(x+i > bounds.second || !game_board.at(x+i).at(x-k+i).is_marker() || game_board.at(x+i).at(x-k+i).piece->color != color)
                        break;
                    i++;
                }
                if(i>length-1)
